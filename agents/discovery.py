...
class InfluencerDiscoveryAgent:
    ...
    async def find_matches(self, campaign_data: CampaignData, max_results: int = 3) -> List[CreatorMatch]:
        ...
        logger.info(f"🔍 Loaded {len(self.creators_data)} creators from file")
        logger.info(f"🔍 Looking for niche: {campaign_data.product_niche}")
        ...
        try:
            ...
            for creator in self.creators_data:
                ...
                # DEBUG: Log detailed scoring for tech influencers
                if creator.niche.lower() == "tech" or "tech" in creator.name.lower():
                    logger.info(f"🔍 DEBUG - {creator.name}:")
                    logger.info(f"   Niche: {creator.niche} vs {campaign_data.product_niche}")
                    logger.info(f"   Similarity: {similarity_score:.3f}")
                    logger.info(f"   Niche Match: {niche_match:.3f}")
                    logger.info(f"   Availability: {availability_score:.3f}")
                    logger.info(f"   Rate Compatible: {rate_compatible} (${estimated_rate})")
                    logger.info(f"   Combined Score: {combined_score:.3f} (threshold: {settings.similarity_threshold})")
                ...
                if combined_score >= settings.similarity_threshold:
                    ...
                    logger.info(f"✅ {creator.name} MATCHED with score {combined_score:.3f}")
                else:
                    logger.info(f"❌ {creator.name} FILTERED OUT - score {combined_score:.3f} < threshold {settings.similarity_threshold}")
    ...
    async def discover_influencers(self, product_niche: str, total_budget: float) -> List[CreatorMatch]:
        """
        ...
        try:
            ...
            logger.info(f"✅ Discovered {len(creator_matches)} influencers")
            return creator_matches
            
        except Exception as e:
            logger.error(f"❌ Discovery failed: {e}")
            
            # Return mock CreatorMatch objects as fallback
            from models.campaign import Creator, CreatorMatch, Platform, Availability
